...
const Home = () => {
	...
	const register = async () => {
		if (isAuthenticated && user) {
			try {
				const dbuser = (
					await axios(
						`http://localhost:3001/api/user?email=${user?.email}`
					)
				).data;
				return
			} catch {
				...
				const res = await axios.post('http://localhost:3001/api/user', newUser)
				return
			}
		}
	};
	...
	return (
		<>
			<div
				className='relative h-screen overflow-y-scroll mb-36'
				ref={containerRef}>
				<Navbar />
				<Filter />
